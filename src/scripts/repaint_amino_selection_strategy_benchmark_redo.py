...
from src.experiment.exp_codon_pattern import build_random_triple_amino_patterns
...
def get_amino_context_from_path(save_path):
    if os.path.exists(save_path):
        amino_patterns = yaml.safe_load(open(save_path, 'r'))
    else:
        amino_patterns = build_random_triple_amino_patterns(N=6, pos=[32, 38, 44], start_codon_pos=26, seed=42,)
        with open(save_path, 'w') as f:
            yaml.dump(amino_patterns, f)
    return amino_patterns

trial_dir = '../../outputs/real_MRL_pred_MFE_260k'
AMINO_PATTERNS = get_amino_context_from_path(os.path.join(trial_dir, 'repaint_strategy_benchmark_redo', 'synonymous_codon_selection_strategy_benchmark_redo.yaml'))
#STRATEGIES = ['init_random', 'init_usage', 'euclidean', 'wasserstein'] + ['usage_weighted_distance'] * 6
STRATEGIES = ['usage_weighted_distance'] * 5 #+ ['init_random', 'init_usage']   #['init_random_fixed'] #+
GAMMAS = [1.0, 0.5, 0.25, ]#[0.01, 0.025, 0.04, 0.05, 0.1] + [None] * 2
...
def refined_amino_constraint_experiment(trial_path):
    ...
    for strategy, gamma in zip(STRATEGIES, GAMMAS):
        ...
        if gamma is None:
            save_path = os.path.join(trial_path, 'repaint_strategy_benchmark_redo', strategy)
        else:
            save_path = os.path.join(trial_path, 'repaint_strategy_benchmark_redo', strategy + f'_gamma_{gamma}')
        ...
        save_experiment_config(
            ...
            extra_config={
                "experiment_describe": "Amino-acid selection benchmark for amino-aicd repaint (ReDo)",
                "strategy": strategy,
                "gamma": gamma,
            }
        )
        for pattern in AMINO_PATTERNS[2:4]:
