...
def get_amino_context_from_path(save_path):
    if os.path.exists(save_path):
        ...
        amino_pattern = {
            'Single_Amino': build_amino_constraint_patterns(pos = [38], num_amino_choices= [19], exclude_amino=('M', '*'), start_codon_pos=26),
            'Double_Amino': build_amino_constraint_patterns(pos = [35, 41], num_amino_choices= [8, 8], start_codon_pos=26),
            'Triple_Amino': build_amino_constraint_patterns(pos = [32, 38, 44], num_amino_choices= [4, 4, 4], start_codon_pos=26),
            'Quadra_Amino': build_amino_constraint_patterns(pos = [29, 35, 41, 47], num_amino_choices= [3, 3, 3, 3], start_codon_pos=26),
        }
...
def refined_amino_constraint_experiment(trial_path):
    ...
    # we need to run the last 8 entries in double amino
    amino_pattern_temp = {k: v[70:] for k, v in AMINO_PATTERNS.items() if k in ['Quadra_Amino']}
