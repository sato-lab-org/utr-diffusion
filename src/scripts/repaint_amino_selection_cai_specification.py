...
AMINO_PATTERNS = get_amino_context_from_path(os.path.join(trial_dir, 'repaint_CAI_specification', '7_amino_pattern_for_cai.yaml'))
STRATEGY = 'specific_adaptiveness'
target_adaptiveness = [0.5, 0.6, 0.7, 0.8, 0.9]
...
def refined_amino_constraint_experiment(trial_path):
    ...
    for adap in target_adaptiveness:
        save_path = os.path.join(trial_path, 'repaint_CAI_specification', STRATEGY + f'_adaptiveness_{adap}')
