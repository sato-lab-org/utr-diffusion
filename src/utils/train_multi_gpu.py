...
class TrainLoop_multi_gpu(BasicTrainLoop):
    ...
    def _condition_labels(self, labels):
        return labels if self.with_condition else None
    ...
    def train(self, epoch):
        ...
        for step, batch in enumerate(self.train_dl):
            ...
            with self.accelerator.autocast():  # Mixed precision on 混合精度オンにする
                labels = self._condition_labels(y)
                loss = self.model(x, labels)
    ...
    def sample(self, epoch, write_fasta=True):
        ...
        snapshot_dir = (
            ...
            else os.path.join('save', self.save_name)
        )
