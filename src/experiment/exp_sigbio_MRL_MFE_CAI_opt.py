...
STRATEGY = 'specific_adaptiveness' #'usage_weighted_distance'
GAMMA = 0.04
target_adaptiveness = [0.85]#[0.85, 0.9, 0.95]
...
def benchmark_MFE_CAI_optimization(trial_path):
    ...
    for adap in target_adaptiveness:
        save_path = os.path.join(trial_path, 'sigbio_exp', f'adaptiveness_{adap}')
        ...
        save_experiment_config(
            ...
            family_name='benchmark_MRL_MFE_CAI_optimization',
            patterns=AMINO_PATTERNS,
            extra_config={
                "experiment_describe": "exp for sigbio86",
                "strategy": STRATEGY,
                "adaptiveness": adap,
                "gamma_for_usage_frequency": GAMMA,
            }
        )
        for original_pattern in AMINO_PATTERNS[20:25]:
            pattern = extend_utr_by_shortening_cds(original_pattern, remove_tail_aa=9) # UTR:26nt CDS: AUG+7aa
            savename = os.path.join(save_path, pattern['name'] + f'_UTR_26nt.fasta')
            if os.path.exists(savename):
                print(f'[INFO] Skip existing pattern: {pattern["name"]}')
                continue
            print(f'[INFO] Sampling for pattern: {pattern["name"]} with amino {pattern["amino"]} at positions {pattern["pos"]} with 5 prime UTR 26nt')
