...
def benchmark_MFE_CAI_optimization(trial_path):
    ...
    for adap in target_adaptiveness:
        ...
        save_experiment_config(
            ...
            extra_config={
                "experiment_describe": "benchmark of MFE-CAI optimization final version, UTR 20nt + AUG + 9aa = 50nt",
                "strategy": STRATEGY,
                "adaptiveness": adap,
                "gamma_for_usage_frequency": GAMMA,
            }
        ...
        for original_pattern in AMINO_PATTERNS:
            for num_aa_shift in range(max_aa_shift):
                pattern = extend_utr_by_shortening_cds(original_pattern, num_aa_shift)
                savename = os.path.join(save_path, pattern['name'] + f'_shift_{num_aa_shift}.fasta')
                if os.path.exists(savename):
                    print(f'[INFO] Skip existing pattern: {pattern["name"]}')
                    continue
                print(f'[INFO] Sampling for pattern: {pattern["name"]} with amino {pattern["amino"]} at positions {pattern["pos"]} with shift {num_aa_shift} AA')
